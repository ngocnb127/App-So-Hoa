...
public class SettingActivity extends UserBaseActivity {
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        btnSave.setOnClickListener(new View.OnClickListener(){
            ...
            public void onClick(View v) {
                ...
                if (!raw.isEmpty()) {
                    try { currentAmount = Float.parseFloat(raw); }
                    catch (NumberFormatException e) { currentAmount = 0f; }
                }
    ...
    private void populateTrucks(List<TruckModel> trucks) {
        ...
        // 1) Tìm vị trí xe đã lưu trong setting (ưu tiên theo Id, fallback theo TruckNo)
        int selectedIndex = 0; // mặc định xe đầu tiên
        if (trucks != null && trucks.size() > 0) {

            Integer savedId = null;
            try {
                savedId = settingModel.getId(); // nếu Id là long primitive thì bỏ try/catch
            } catch (Exception ignored) {}

            String savedTruckNo = null;
            try {
                savedTruckNo = settingModel.getTruckNo();
            } catch (Exception ignored) {}

            // Ưu tiên match theo Id
            if (savedId != null && savedId > 0) {
                for (int i = 0; i < trucks.size(); i++) {
                    TruckModel t = trucks.get(i);
                    if (t != null && t.getId() != null && t.getId().longValue() == savedId.longValue()) {
                        selectedIndex = i;
                        break;
                    }
                }
            } else if (savedTruckNo != null && savedTruckNo.trim().length() > 0) {
                // Fallback match theo TruckNo
                for (int i = 0; i < trucks.size(); i++) {
                    TruckModel t = trucks.get(i);
                    if (t != null && t.getTruckNo() != null &&
                            t.getTruckNo().trim().equalsIgnoreCase(savedTruckNo.trim())) {
                        selectedIndex = i;
                        break;
                    }
                }
        ...
        truckSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (model != null) {
                    ...
                    ((EditText) findViewById(R.id.txtCurrentAmount))
                            .setText(String.format("%.0f", model.getCurrentAmount()));
                }
            }
        ...
        // 3) Set selection: nếu có xe lưu thì chọn xe đó, không có thì chọn xe đầu tiên
        if (trucks != null && trucks.size() > 0) {
            // setSelection(..., false) để tránh gọi onItemSelected 2 lần trên một số máy
            truckSpinner.setSelection(selectedIndex, false);

            // Nhưng vẫn cần set dữ liệu vào UI + settingModel 1 lần theo item đang chọn
            TruckModel model = trucks.get(selectedIndex);
            if (model != null) {
                settingModel.setTruckNo(model.getTruckNo());
                settingModel.setId(model.getId());
                settingModel.setCapacity(model.getCapacity());
