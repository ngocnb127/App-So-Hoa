...
public class BM2508Model extends BaseModel {
    ...
    private transient DensitySource densitySource = null;
    private transient boolean userEditedDensity = false;
    public boolean isUserEditedDensity() {
        return densitySource == DensitySource.USER;
    }
    public enum DensitySource {
        USER,           // user nhập tay
        LATEST,         // lấy từ bản ghi BM2508 gần nhất
        DEFAULT_08      // fallback 0.790
    }
    ...
    public void applyLatestDensity(double latestDensity) {
        ...
        // 🔥 QUAN TRỌNG: user đã nhập → KHÔNG override
        if (isUserEditedDensity()) return;

        if (latestDensity >= 0.72 && latestDensity <= 0.86) {
    ...
    public void recalcFuelOnBoardGallon() {
        ...
        double uplift = FuelOnBoard - BeforeFueling;
        ...
        if ("USG".equalsIgnoreCase(Unit)) {
            FuelOnBoardGallon = Math.round(FuelUplift);
            return;
        }

        if ("KGs".equalsIgnoreCase(Unit)) {
            if (density <= 0) applyDefaultDensity();
            double liter = FuelUplift / density;
            FuelOnBoardGallon = Math.round(liter / GALLON_TO_LITTER);
            return;
        }

        if ("LBs".equalsIgnoreCase(Unit)) {
            if (density <= 0) applyDefaultDensity();
            double usg = FuelUplift / (density * 8.345);
            FuelOnBoardGallon = Math.round(usg);
        }
    }
    ...
    public String getDensityDisplayText() {
        if (densitySource == DensitySource.LATEST) {
            return String.format("Đang áp dụng tỷ trọng mới nhất (%.3f)", density);
        }
        return "Đang áp dụng tỷ trọng mặc định (0.790)";
    }

    public String getDensityText() {
        return density > 0 ? String.format("%.3f", density) : "";
    }

    public void setDensityText(String value) {
        ...
        try {
            applyLatestDensity(Double.parseDouble(value));
        } catch (Exception e) {
            applyDefaultDensity();
        }
    }

    // 🔥 CHỖ DUY NHẤT SET density từ UI
    // ⭐ DUY NHẤT – CHỖ USER NHẬP
    // BM2508Model
    public void updateDensityFromUser(double value) {
        this.density = value;
        this.densitySource = DensitySource.USER;
        this.userEditedDensity = true;
        recalcFuelOnBoardGallon();
    }
